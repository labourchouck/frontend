...
export function AdminVendorBannersPage() {
  ...
  const loadBanners = async () => {
    try {
      setLoading(true)
      const res = await fetchAdminBanners('VENDOR')
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Vendor Banners</h1>
          <p className="mt-1 text-sm text-slate-500">
            Upload banners that will be visible on the Vendor dashboards.
          </p>
        </div>
        <input
          type="file"
