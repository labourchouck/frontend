...
import { fetchAdminMartProducts, createAdminMartProduct, deleteAdminMartProduct } from '../../api/adminBuildmartApi.js'
import { ApiError } from '../../api/http.js'
import { GlassPanel } from '../../components/ui/GlassPanel.jsx'
import { AppPrimaryButton } from '../../components/app/AppPrimaryButton.jsx'
import { EditMartProductModal } from '../../components/admin/EditMartProductModal.jsx'

export function AdminMartProductsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const data = await fetchAdminMartProducts()
      setItems(data)
    } catch (err) {
  ...
  async function handleDelete(id) {
    ...
    try {
      await deleteAdminMartProduct(id)
      load()
    } catch(e) {
      alert(e.message)
    }
  ...
  return (
    <motion.div initial={reduce ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between mb-4">
        <div><h2 className="text-xl font-extrabold text-slate-900">Mart Products</h2></div>
        <div className="flex gap-2">
          <button onClick={load} className="btn-secondary px-3 py-2"><RefreshCw className="h-4 w-4" /></button>
          <AppPrimaryButton onClick={() => setIsAddOpen(true)} className="py-2"><Plus className="h-4 w-4 mr-1" /> Add Product</AppPrimaryButton>
        </div>
      </div>
