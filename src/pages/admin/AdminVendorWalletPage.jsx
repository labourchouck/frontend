...
export function AdminVendorWalletPage() {
  ...
  const [stats, setStats] = useState({ totalUnpaid: 0, pendingAmount: 0, totalPaidOut: 0, vendorBalances: [] })
    ...
    const fetchData = async () => {
      ...
      try {
        const [statsRes, wRes] = await Promise.all([
          adminWalletsApi.getVendorWalletStats(),
          adminWalletsApi.getVendorWithdrawals()
        ])
  ...
  const handleUpdateStatus = async (id, status, remarks = '') => {
    ...
    try {
      ...
      const statsRes = await adminWalletsApi.getVendorWalletStats()
  ...
  const { totalUnpaid, pendingAmount, totalPaidOut, vendorBalances } = stats
  ...
  const historyWithdrawals = useMemo(() => {
    return withdrawals.filter(w => {
      ...
      if (searchTerm) {
        ...
        const nameMatch = w.vendorId?.fullName?.toLowerCase().includes(lowerSearch)
  ...
  return (
    <div className="space-y-6 relative">
      <div className="mb-8">
        <h1 className="text-2xl font-extrabold tracking-tight text-slate-900">Vendor Wallet</h1>
        <p className="text-sm text-slate-500">Manage vendor balances, process payouts, and view history.</p>
      </div>
                  ...
                  {pendingWithdrawals.map(req => (
                    <tr key={req._id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-4">
                        <p className="font-bold text-slate-900">{req.vendorId?.companyName || req.vendorId?.fullName || 'N/A'}</p>
                        <p className="text-xs text-slate-500">{req.vendorId?.phone || ''}</p>
                      </td>
                      <td className="px-6 py-4">
                ...
                currentHistory.map(w => (
                  <tr key={w._id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium text-slate-700">
                      {w.vendorId?.companyName || w.vendorId?.fullName || 'N/A'}
                    </td>
